		...
		if (isset($_POST['simpan'])) {
			...
			while ($rowx4 = $stmtx4->fetch()) {
				if ($rowx4['id_kriteria'] == true) {
					...
					$stmt2->execute();
					$getminmax = GetMinMax($rowx4["id_kriteria"], $db);
					if ($getminmax) {
						$stmt6 = $db->prepare("update smart_kriteria set min=? , max=? where id_kriteria=?");
						$stmt6->bindParam(1, $getminmax[0]);
						$stmt6->bindParam(2, $getminmax[1]);
						$stmt6->bindParam(3, $rowx4['id_kriteria']);
						$stmt6->execute();
					} else {
						$stmt6 = $db->prepare("update smart_kriteria set min=? , max=? where id_kriteria=?");
						$stmt6->bindParam(1, $_POST['altkri'][$rowx4['id_kriteria']]);
						$stmt6->bindParam(2, $_POST['altkri'][$rowx4['id_kriteria']]);
						$stmt6->bindParam(3, $rowx4['id_kriteria']);
						$stmt6->execute();
					}
				}
...
<form method="post">
	...
	?>
		<div class="row cells3">
			<div class="cell"><input type="hidden" name="kri[<?php echo $row4['id_kriteria'] ?>]" value="<?php echo $row4['id_kriteria'] ?>">
				<?php echo $no++ ?>.
			...
			<div class="cell colspan2">
				<div class="input-control select full-size">
					<select name="altkri[<?php echo $row4['id_kriteria'] ?>]">
						<?php
						$stmt5 = $db->prepare("select * from smart_sub_kriteria where id_kriteria='" . $row4['id_kriteria'] . "' ORDER BY nilai_sub_kriteria DESC ");
						$stmt5->execute();
...
?>
	...
	<?php
		if (isset($_GET['alt'])) {
			...
			if ($stmt->execute()) {
				$stmtx9 = $db->prepare("select id_kriteria from smart_kriteria");
				$stmtx9->execute();
				foreach ($stmtx9->fetchAll() as $data) {
					$getminmax = GetMinMax($data["id_kriteria"], $db);
					if ($getminmax) {
						$stmt6 = $db->prepare("update smart_kriteria set min=? , max=? where id_kriteria=?");
						$stmt6->bindParam(1, $getminmax[0]);
						$stmt6->bindParam(2, $getminmax[1]);
						$stmt6->bindParam(3, $data["id_kriteria"]);
						$stmt6->execute();
					} else {
						$stmt6 = $db->prepare("update smart_kriteria set min=? , max=? where id_kriteria=?");
						$null = null;
						$stmt6->bindParam(1, $null);
						$stmt6->bindParam(2, $null);
						$stmt6->bindParam(3, $data["id_kriteria"]);
						$stmt6->execute();
					}
				}
	...
	?>
	<div class="cell colspan2 align-right">
		<a href="execute-rangking.php" class="button success">Eksekusi Perhitungan</a>
